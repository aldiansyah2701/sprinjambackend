...
public interface RoleRepository extends CrudRepository<Role, String> {
	...
	@Query(nativeQuery = true, value = "select u.name, u.uuid as uuid, ( select u2.is_active from users u2 where u2.uuid = u.uuid) as active, GROUP_CONCAT(r.name separator ';') as roles from roles r join users u on r.user_uuid = u.uuid group by r.user_uuid")
